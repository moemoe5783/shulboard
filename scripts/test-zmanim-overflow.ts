...
const BOX = { boxHeight: 400, boxWidth: 1920, canvasWidth: 1920 };
...
for (const mode of ["page", "scroll", "clip"] as const) {
  const state = overflowState({ mode, second: 12_345, ...SEVEN });
  check(!state.overflowing && state.offset === 0 && !state.animate,
    `${mode}: a table that fits is perfectly still — no offset, no transition`,
    `offset ${state.offset}`);
}
check(
  !overflowState({ mode: "page", second: 12_345, ...BOX, contentHeight: 401, rowCount: 10 }).overflowing,
...
check(
  overflowState({ mode: "page", second: 12_345, ...BOX, contentHeight: 402, rowCount: 10 }).overflowing,
  "two pixels over is",
);
...
check(
  !overflowState({ mode: "scroll", second: 100, boxHeight: 0, boxWidth: 0, canvasWidth: 1920, contentHeight: 0, rowCount: 12 })
    .overflowing,
  "unmeasured (zero heights) reads as fitting, so the first frame is a still, complete table",
);
check(
  overflowState({ mode: "scroll", second: null, ...TWELVE }).offset === 0,
...
{
  const state = overflowState({ mode: "clip", second: 12_345, ...TWELVE });
...
{
  const first = overflowState({ mode: "page", second: 0, ...TWELVE });
  ...
  const second = overflowState({ mode: "page", second: PAGE_SECONDS, ...TWELVE });
  ...
  const wrapped = overflowState({ mode: "page", second: PAGE_SECONDS * 2, ...TWELVE });
...

{
  ...
  const state = overflowState({ mode: "page", second: 0, ...BOX, contentHeight: 504, rowCount: 12 });
...

{
  ...
  const tiny = overflowState({ mode: "page", second: 0, ...BOX, boxHeight: 20, contentHeight: 504, rowCount: 12 });
...

{
  ...
  // units per second.
  const a = overflowState({ mode: "scroll", second: 1000, ...TWELVE });
  const b = overflowState({ mode: "scroll", second: 1001, ...TWELVE });
  check(b.offset - a.offset === SCROLL_UNITS_PER_SECOND,
...

{
  ...
  const half = overflowState({ mode: "scroll", second: 1000, ...TWELVE, boxWidth: 960 });
  const full = overflowState({ mode: "scroll", second: 1000, ...TWELVE });
  const halfNext = overflowState({ mode: "scroll", second: 1001, ...TWELVE, boxWidth: 960 });
  check(halfNext.offset - half.offset === SCROLL_UNITS_PER_SECOND / 2,
...

{
  ...
  const cycleSeconds = 504 / SCROLL_UNITS_PER_SECOND; // 31.5s
  const justWrapped = overflowState({ mode: "scroll", second: Math.ceil(cycleSeconds), ...TWELVE });
  ...
  const middle = overflowState({ mode: "scroll", second: Math.floor(cycleSeconds / 2), ...TWELVE });
  check(!middle.wrapped, "mid-cycle is not", middle.offset);

  // And the wrap really does return to near-zero rather than running away.
  const many = overflowState({ mode: "scroll", second: 100_000, ...TWELVE });
...

{
  ...
  check(!overflowState({ mode: "scroll", second: 0, ...SEVEN }).wrapped, "a fitting table is never 'wrapped'");
}
