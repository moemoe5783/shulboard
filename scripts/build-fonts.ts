...
import { ENGLISH_FONTS, HEBREW_FONTS, HEBREW_MARKS_FALLBACK, HEBREW_PUNCTUATION } from "../lib/fonts/catalog.ts";
...
for (const font of [...ENGLISH_FONTS, ...HEBREW_FONTS]) {
  ...
  if (!b.digits) flags.push(`${font.name}: no complete digit set`);
    ...
    flags.push(`${font.name}: old-style figures, no lining set — times are set in ${fallback}`);
  }
}
