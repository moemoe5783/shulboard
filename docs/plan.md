...
> **READER UPDATE (current): two published sources, not `Get_Zmanim`.**
> `lib/zmanim/warm.ts` merges two into `zmanim_cache`:
>  - **Daily zmanim table** — the RSS feed (`lib/zmanim/chabad-rss.ts`),
>    `chabad.org/tools/rss/zmanim.xml?locationId=<zip>&locationType=2`. **US
>    ZIP only, one day per request.** Chosen because it carries the
>    transliterated Hebrew zman names ("English (Transliteration)") the widget
>    offers. The one-day span means a screen offline for more than a day shows
>    the unavailable state for the new date until it reconnects.
>  - **Candle lighting / Shabbos ends** — the published embed
>    (`lib/zmanim/chabad-embed.ts`),
>    `.../candlelighting.js.asp?locationid=<zip>&locationtype=2&ln=2&weeks=4`.
>    **Four weeks** (chabad.org caps `weeks` at 4). This is what keeps the
>    Candle Lighting widget's upcoming view populated days ahead, which the
>    one-day RSS feed alone cannot. Best-effort: if it fails the daily zmanim
>    still cache.
>
> The 92-day `Get_Zmanim` reader (`lib/zmanim/chabad-adapter.ts`) described
> throughout this section is **kept unwired** for the day the whole span has to
> come from one request again; everything below about its shapes, parameters
> and 92-day span describes that dormant reader, not current runtime. The
> Zmanim widget's label options are now **English** and **transliterated
> Hebrew** (both the provider's own words); Hebrew script is not shown, because
> the RSS feed does not carry it.
>
